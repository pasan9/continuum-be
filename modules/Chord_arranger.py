...
creator.create("ChoIndividual", list, fitness=creator.Fitness)

def get_chord_placements(guitar,segment,previous_result):
    ...
    def evalInd(individual):
        ...
        #Maximizing String Diversity
        #Get list of string values
        string_vals = list(list(i[0] for i in individual))

        #Get number of unique strings
        string_diversity = len(np.unique(string_vals))

        return total_fitness,string_diversity
    ...
    toolbox.register("individual", tools.initIterate, creator.ChoIndividual, create_individual)
